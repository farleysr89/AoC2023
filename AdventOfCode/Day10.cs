...
public class Day10 : BaseDay
{
    ...
    public bool TryRight(int x, int y, char[][] map)
    {
        ...
        if (map[y][x] != '.' && map[y][x] != 'L' && map[y][x] != 'F')
            return true;
        return false;
    }
    ...
    public bool TryLeft(int x, int y, char[][] map)
    {
        x--;
        if (x < 0) return false;
        if (map[y][x] != '.' && map[y][x] != '7' && map[y][x] != 'J')
            return true;
        return false;
    }
...
var total = 0;
        ...
        while (next != start)
        {
            ...
            area += ((prev.Item1 * next.Item2) - (next.Item1 * prev.Item2));
            prev = next;
        }
        area += ((prev.Item1 * next.Item2) - (next.Item1 * prev.Item2));
        ...
        var interiorPoints = area + 1 - (total);
        bool outOfLoop = true; bool inPipe = false;
        //foreach(var line in steps)
        //{
        //    foreach(var s in line)
        //    {
        //        if(s != int.MaxValue) { outOfLoop = !outOfLoop; continue;}
        //        if(!outOfLoop) total++;
        //        inPipe = false;
                
        //    }
        //}
        //for(int yy = 0;yy < steps.Count(); yy++)
        //{
        //    for(int x = 0; x < steps[yy].Count(); x++)
        //    {
        //        if(steps[yy][x] != int.MaxValue) { outOfLoop = !outOfLoop; continue;}
        //        if(!outOfLoop) total++;
        //        inPipe = false;
        //    }
        //}
        return new($"Solution to {ClassPrefix} {CalculateIndex()}, part 2 is " + interiorPoints);
    }
